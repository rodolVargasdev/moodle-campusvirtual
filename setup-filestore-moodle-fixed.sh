...
echo "🚀 CONFIGURACIÓN COMPLETA DE MOODLE ESCALABLE CON FILESTORE (CORREGIDA)"
echo "======================================================================"
...
echo "🔍 Verificando si Filestore ya existe..."
if gcloud filestore instances describe $FILESTORE_NAME --zone=$ZONE &>/dev/null; then
    echo "✅ Filestore ya existe, obteniendo IP..."
...
if [ -z "$FILESTORE_IP" ]; then
    echo "❌ Error: No se pudo obtener la IP del Filestore"
    exit 1
fi
...
sleep 10
PVC_STATUS=$(kubectl get pvc moodle-filestore-pvc -n moodle -o jsonpath='{.status.phase}' 2>/dev/null)
if [ "$PVC_STATUS" != "Bound" ]; then
    echo "❌ Error: PVC no está vinculado. Estado: $PVC_STATUS"
    kubectl describe pvc moodle-filestore-pvc -n moodle
    exit 1
fi
echo "✅ PVC vinculado correctamente"
...
while [ $attempt -le $max_attempts ]; do
    ...
    if [ "$ready_pods" -eq "$total_pods" ] && [ "$total_pods" -gt 0 ]; then
        ...
        if [ $attempt -gt 5 ]; then
            echo "🔍 Mostrando logs de pods con problemas..."
...
if [ $attempt -gt $max_attempts ]; then
    ...
    echo "   kubectl describe pods -l app=moodle-scalable -n moodle"
